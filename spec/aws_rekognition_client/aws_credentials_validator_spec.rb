...
RSpec.describe AwsRekognitionClient::AwsCredentialsValidator do
  context '.validate!' do
    context 'when access key is blank' do
      let(:instance) { instance_double(AwsRekognitionClient::Configuration, access_key: nil, secret_key: Faker::Crypto.sha1) }
    ...
    context 'when secret key is blank' do
      let(:instance) { instance_double(AwsRekognitionClient::Configuration, secret_key: nil, access_key: Faker::Crypto.sha1) }
